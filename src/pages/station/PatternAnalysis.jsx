...
const StationPatternAnalysis = () => {
  ...
  const filteredCases = useMemo(() => {
    ...
    return result
  }, [cases, caseSearch, statusFilter])

  // ── Categories & cities from station data ──
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (mode === 'existing') {
        if (!selectedCase) {
          toast.error('Select a case from your station cases')
          setSearching(false)
          return
        }
    ...
    <Layout>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-base-muted mt-1">
              AI-powered crime pattern detection — analyze all cases assigned to your station.
            </p>
          </div>
      ...
      <div className="stat-grid mb-6">
        <div className="stat-card">
          ...
          <div className="stat-label">Total Station Cases</div>
      ...
      <div className="card p-6 mb-6">
          ...
          <div className="space-y-4">
            <div className="flex gap-3">
              <div className="flex-1">
                <label className="input-label">Search Station Cases</label>
                <input
              ...
              <div className="w-44">
                <label className="input-label flex items-center gap-1"><FunnelIcon className="h-3 w-3" /> Status</label>
            ...
            {loadingCases ? (
              <p className="text-xs text-base-muted">Loading station cases...</p>
            ) : (
              <div className="max-h-56 overflow-y-auto rounded-xl border border-base-border bg-base-panelAlt">
                {filteredCases.length === 0 ? (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="ml-2 text-xs text-base-muted">{c.category}</span>
                      {c.description && (
                        <p className="text-[10px] text-base-muted truncate mt-0.5">{c.description.substring(0, 80)}...</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2 shrink-0 ml-3">
                      <StatusChip status={(c.status || 'new').toLowerCase()} />
        ...
        <div className="mt-5 pt-4 border-t border-base-border">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-2 pt-5">
              ...
              />
              <label htmlFor="sameCityOnlyStation" className="text-sm text-base-muted">Same city only</label>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end mt-5">
          <button
            ...
            disabled={searching || apiOnline === false}
          >
            {searching ? (
      ...
      <div className="card p-4 text-[11px] text-base-muted">
        <p>
          <strong>🔒 Security:</strong> Only cases assigned to your station are displayed and analyzable.
          {' '}This ensures data isolation between stations.
        </p>
        <p className="mt-1">
          <strong>🧠 AI Engine:</strong> Powered by SentenceTransformer (all-MiniLM-L6-v2) with multi-modal similarity scoring
          — NLP semantics, crime category matching, geographic proximity, and temporal patterns.
        </p>
      </div>
    </Layout>
  )
...
export default StationPatternAnalysis
